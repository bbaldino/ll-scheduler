...
export async function createCageAvailability(
  ...
  await db
    .prepare(
      `INSERT INTO cage_availabilities (id, season_cage_id, day_of_week, start_time, end_time, single_event_only, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
    )
    .bind(id, input.seasonCageId, input.dayOfWeek, input.startTime, input.endTime, singleEventOnly, now, now)
    .run();
